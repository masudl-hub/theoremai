...
- **Session events (done):** setup, resumption (a handle is a credential: recorded only as issued), voice activity, `goAway`, waiting / working / idle, and `closed {code, reason, initiator}` are events on the session span. The session status is ERROR when something was thrown or the provider closed with a code other than 1000 without warning first (`error.type` is the close's kind, and the host receives an `error` event). A close after `goAway` is UNSET with `theorem.stop.kind=go_away`, whatever its code; the `closed` event keeps the code, reason, the code's kind and the warning's timing, and the host receives a `session` `ended` event. `theorem.stop.kind` is otherwise set only when the host cancelled.
