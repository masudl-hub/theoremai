...
function shouldSkipStreamEvent(event: TurnEvent, profile: Profile): boolean {
  return (
    event.type === "thought" &&
    profileTurnOutputs(profile)?.streaming?.streamThoughts === false
  );
}
...
async function* yieldProviderEvents(args: {
  ...
  const { profile, generation, system, provider, upstream, signal, control } =
    args;
  ...
  for await (
    const event of provider.complete({
